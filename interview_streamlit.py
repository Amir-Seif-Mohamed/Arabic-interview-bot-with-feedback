...
Please provide feedback in Arabic language on the candidate's performance in the interview. Even if their resume is great
...
INITIAL_TRANSCRIPT = "مرحبا بك كيف حالك؟"
...
Only use Arabic
The interview should adhere to the following format In Arabic:
...
def main():
    ...
    if "transcript" not in session:
        ...
        text2speech.text_speech(INITIAL_TRANSCRIPT)
    ...
    with chat_tab:
        ...
        def clear_text():
            ...
            session["candidate_text"] = ""
        def record():
            user_text = speech2text.specch_to_text(audiofile='',rec=1)
            session.transcript.append(f"Candidate: {user_text.strip()}")            
        ...
        record_button = st.button("Record", help = "Press to start recording", on_click=record)
        text = st.button("Chat", help="Submit your chat", on_click=clear_text)
        run_button =  st.button("Enter", help="Submit your chat/record", on_click=clear_text)
        if run_button:
            ...
            text2speech.text_speech(completion_text)
